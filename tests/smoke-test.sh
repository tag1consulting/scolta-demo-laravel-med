...
# them. The test is only worth its runtime if what it exercises is what
# production runs, so it runs the chart's environment and the chart's postDeploy
# commands.
#
...
# `database`. The second path used to work here only because the committed dump
# happens to carry a sessions table; create_sessions_table now creates it, and
# this assertion is what proves the guarded migration is applied rather than
# assumed.
set -euo pipefail
