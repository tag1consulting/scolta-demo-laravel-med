...
<section class="relative overflow-hidden bg-lunar-950 border-b border-lunar-800">
    ...
    <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 sm:py-28">
        <div class="max-w-3xl">
            <p class="text-xs tracking-widest uppercase text-blue-400 mb-4 font-medium">Lunar Medical Reference</p>
            <h1 class="text-4xl sm:text-5xl lg:text-6xl font-bold text-lunar-50 leading-tight mb-6">
                Medical care,<br>
                <span class="text-transparent bg-clip-text bg-gradient-to-r from-lunar-200 to-lunar-400">384,400 km from home.</span>
            </h1>
            <p class="text-lg text-lunar-300 mb-8 max-w-2xl">
            ...
            <form action="{{ route('search') }}" method="GET" class="flex gap-3 max-w-xl">
                <div class="flex-1 relative">
                    ...
                    <input
                        ...
                        class="w-full pl-12 pr-4 py-3.5 bg-lunar-900 border border-lunar-700 rounded-xl text-lunar-100 placeholder-lunar-500 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 text-sm"
                    >
                </div>
                <button type="submit" class="px-6 py-3.5 bg-blue-600 hover:bg-blue-500 text-white font-medium rounded-xl transition-colors text-sm whitespace-nowrap">
                    Search
                </button>
            </form>
            ...
            <div class="mt-4 flex flex-wrap gap-2">
                ...
                <a href="{{ route('search') }}?q={{ urlencode($q) }}" class="text-xs text-lunar-500 hover:text-lunar-300 transition-colors px-2.5 py-1 rounded-full border border-lunar-800 hover:border-lunar-600">
                    {{ $q }}
                </a>
                @endforeach
            </div>
        ...
        <div class="mt-12 grid grid-cols-2 sm:grid-cols-4 gap-4 max-w-2xl">
            ...
            <a href="{{ $stat['url'] }}" class="bg-lunar-900/60 border border-lunar-800 rounded-lg p-4 hover:border-lunar-600 transition-colors group text-center">
                <div class="text-2xl font-bold text-lunar-100 group-hover:text-white transition-colors">
                    {{ number_format($stat['count']) }}
                </div>
                <div class="text-xs text-lunar-500 mt-1">{{ $stat['label'] }}</div>
            </a>
            @endforeach
        </div>
    </div>
...
<section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-8">
    <div class="card-emergency rounded-xl p-6 flex items-start gap-4">
        <div class="text-red-400 text-2xl flex-shrink-0">⚠</div>
        <div class="flex-1 min-w-0">
            <h2 class="text-base font-semibold text-red-300 mb-2">Emergency Protocols</h2>
            <p class="text-sm text-red-200/70 mb-4">For life-threatening emergencies, contact Earth Telemedicine (+1.3s relay delay) and initiate evacuation assessment. All emergency conditions are listed below.</p>
            @if($emergencyConditions->isNotEmpty())
            <div class="flex flex-wrap gap-2">
                @foreach($emergencyConditions as $cond)
                <a href="{{ route('conditions.show', $cond->slug) }}" class="text-xs px-3 py-1.5 bg-red-900/50 border border-red-800 rounded-full text-red-300 hover:bg-red-800/50 transition-colors">
